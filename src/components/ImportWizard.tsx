  ...
  return (
    <div className="grid" style={{ gap: "1rem" }}>
      <section className="panel">
        ...
        <p className="lede">CSV, OFX/QFX, PDF, or TXT. Preview before commit. Duplicates are skipped.</p>
        ...
        <div className="grid cols-3" style={{ marginTop: "1rem" }}>
          <div className="field">
            ...
            <select id="preset" value={preset} onChange={(e) => setPreset(e.target.value)}>
              {(preview?.presets ?? ["generic", "chase", "amex", "capital-one", "bank-of-america"]).map(
                (p) => (
        ...
        <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.5rem" }}>
          ...
          <button className="btn" type="button" disabled={busy || !preview} onClick={commit}>
            Commit import
          </button>
        </div>
      </section>
      ...
      {preview ? (
        <section className="panel">
          <h2>
            Preview · {preview.format.toUpperCase()} · {preview.rows.length} rows · net{" "}
            {(total / 100).toFixed(2)}
          </h2>
